...
SELECT P.NUMERO, 207 AS CODIGO_PRO, 100 AS QUANTIDADE, 10.00 AS PRECO_UNITARIO
FROM PEDIDO P
WHERE MOD(P.NUMERO, 2) = 0
AND EXTRACT(YEAR FROM P.DATA) < 2018;

INSERT INTO ITEM_PRODUTO (NUMERO_PED, CODIGO_PRO, QUANTIDADE, PRECO_UNITARIO)
SELECT P.NUMERO, 206 AS CODIGO_PRO, 50 AS QUANTIDADE, 12.00 AS PRECO_UNITARIO
FROM PEDIDO P
WHERE MOD(P.NUMERO, 2) != 0
AND EXTRACT(YEAR FROM P.DATA) = 2018;

INSERT INTO ITEM_PRODUTO (NUMERO_PED, CODIGO_PRO, QUANTIDADE, PRECO_UNITARIO)
SELECT P.NUMERO, 207 AS CODIGO_PRO, 150 AS QUANTIDADE, 14.00 AS PRECO_UNITARIO
FROM PEDIDO P
WHERE MOD(P.NUMERO, 2) = 0
AND EXTRACT(YEAR FROM P.DATA) = 2018;
...
FROM PEDIDO
WHERE 
     MOD(TO_CHAR(PRAZO_ENTREGA, 'DD'), 2) != 0;
